...
def _result_picker(key, results):
...
class NYPLsearch(object):
    ...
    def captures(self, uuid, withTitles=False):
        ...
        return self._get(url, picker, withTitles='yes' if withTitles else 'no')

    # Return the item-uuid for a identifier.
    def uuid(self, type, val):
        picker = lambda x: _result_picker('uuid', x)
        return self._get('/'.join((self.base, type, val)), picker)
    ...
    def search(self, q, field=None, page=None, per_page=None):
        ...
        picker = lambda x: _result_picker('result', x)
        return self._get(url, picker, q=q, field=field, page=page, per_page=per_page)

    # Return a mods record for a given uuid
    def mods(self, uuid):
        picker = lambda x: x.get('mods')
        return self._get('/'.join((self.base, 'mods', uuid)), picker)

    # Generic get which handles call to api and setting of results
    # Return: results dict
    def _get(self, url, picker, **params):
        ...
        if self.request.get('totalPages') > self.request.get('page'):
            del params['page']
            self.next_page = lambda x: self._get(url, picker, page=self.request.get('page', 0) + 1, **params)
        else:
            self.next_page = stop
